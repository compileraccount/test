...
// Copyright (c) 2014 The rushcoin developers
...
uint256 hashGenesisBlock("0x3169d0c7ff1c5e3dd7bc97759caea12c0e24ec51af8bb85f99e1f145b9f3634f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // rushcoin: starting difficulty is 1 / 2^12
...
{
    // rushcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 60; // rushcoin: 1 minute retarget
...
{
        static const int64 BlocksTargetSpacing = 2 * 60; // 2 minutes for rushcoin
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // rushcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // rushcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
		const char* pszTimestamp = "I pray that all the passengers aboard MH370 are alive and well";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5.5 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1395186057; // epoch time;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 297255; // nonce set;

        if (fTestNet)
        {
            block.nTime    = 1395186057;
            block.nNonce   = 0;
        }
	...
	assert(block.hashMerkleRoot == uint256("0x04d0d2be0414be21be61f03687b3eddf1fc7b17d602d6cb90e8b5d754dea459f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0x14, 0x92, 0x00 }; // rushcoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpaincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpaincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpaincoinMiner(CWallet *pwallet)
{
    printf("SpaincoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SpaincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpaincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpaincoinMiner, pwallet));
}
